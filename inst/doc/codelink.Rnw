...
\Rfunction{readCodelink} returns and object of class ``Codelink'':
...
The default method used is based on the \emph{half} method from \emph{limma} 
\cite{limma} package. Median background intensity (Bmedian) is subtracted
