...
and they are in your working directory. The function emph{readCodelink()}
...
> data <- readCodelink(files, flag=list(M=0.01) )
...
The function plotMA() can highlight spots based on Probe\_type values
...
The mergeArray() function merge various normalized arrays of the same
...
function plotCV(). The example above will return the mean of array1
