...
and they are in your working directory. If this is not the case you can specify files to be read with the 'file' argument. The function emph{readCodelink()}
...
The Chip type is read from the PRODUCT field in the header of Codelink files. If not found then a warning is shown. If on product type disagree with the others an error message is shown and reading of files terminated.
