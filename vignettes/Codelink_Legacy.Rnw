...
probe type and flag labels. This can be used during normalization (CyclicLoess) and when fitting a linear model with \Rpackage{limma}.

<<createWeights, comment=NA>>=
w <- createWeights(codelink.example)
